...
item_drop = {
   ...
   can_pickup = function(entity, _player)
   ...
   before_collect = function(_entity, _pos, _player)
   end,
   after_collect = function(entity, _pos, _player)
      entity.item_drop_picked = true
   end,
}

local function legacy_setting_getbool(name_new, _name_old, default)
...
local function legacy_setting_getnumber(name_new, name_old, default)
   return tonumber(minetest.settings:get(name_new)) or
   tonumber(minetest.settings:get(name_old)) or
   default
end
...
if legacy_setting_getbool("item_drop.enable_item_pickup",
   ...
   if magnet_mode then
      ...
      afterflight = function(object, inv, player)
         ...
         if inv and
            ...
            item_drop.can_pickup(ent, player) then
...
"setacceleration", "setvelocity", }
   ...
   elseif keytype == "LeftAndRight" then
      keytype = 0
   elseif keytype == "SneakAndRMB" then
      keytype = 1
   end
...
if legacy_setting_getbool("item_drop.enable_item_drop", "enable_item_drop", true) and
   ...
   local function spawn_items(pos, items_to_spawn)
      for i = 1, #items_to_spawn do
         ...
         if not obj then
            error("Couldn't spawn item " .. ItemStack(items_to_spawn[i]):get_name())
         end
   ...
   minetest.handle_node_drops = function(pos, drops, player)
      ...
      for i = 1, #drops do
         ...
         if name == "" or
            not minetest.registered_items[name] then
            count = 0
         end
